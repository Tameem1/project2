...
# Load your Stripe credentials from env or set default for testing:
...
# Example product IDs and associated plan info:
PLANS = {
    "prod_Rn3o1Du3ssKMNv": {
        "name": "Basic",
...
def create_checkout_session(data: dict, db: Session = Depends(get_db)):
    ...
    # 1) Lookup the local DB 'Customer'
...
async def stripe_webhook(request: Request, db: Session = Depends(get_db)):
    ...
    Stores plan_name in the customers table, and updates usage_tokens
    to reflect the plan's token allotment.
    ...
    if event_type in ["customer.subscription.created", "customer.subscription.updated"]:
        ...
        # Use JSONB operator ->> to match "stripe_customer_id"
        ...
        if plan_data:
            ...
            new_token_allotment = 1000  # fallback for unknown product
        ...
        # Also fix for cancelled/deleted lookup
        db_customer = db.query(Customer).filter(
            Customer.billing_info.op("->>")("stripe_customer_id") == stripe_customer_id
        ).first()
