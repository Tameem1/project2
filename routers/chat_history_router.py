...
class ChatHistoryEntry(BaseModel):
    ...
    question: str
    answer: str
    timestamp: str  # store as string (isoformat)
...
def retrieve_chat_history(
    ...
    Retrieves the chat history for a specific chatbot, 
    validating that it belongs to 'customer_id' in both 
    the route and the JWT token.
    """
    token_customer_id = user.get("customer_id")
    if not token_customer_id or token_customer_id != customer_id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Unauthorized access to this customer's chat history."
        )
    ...
    if chat_history is None:
        raise HTTPException(
            ...
            detail="No chat history found for the specified chatbot."
        )
...
def clear_chat_history(
    ...
    validating it belongs to 'customer_id' from the token.
    """
    token_customer_id = user.get("customer_id")
    if not token_customer_id or token_customer_id != customer_id:
        raise HTTPException(
    ...
    if not success:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Chat history not found for the specified chatbot."
        )